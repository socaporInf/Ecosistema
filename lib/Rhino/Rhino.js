...
Cabecera.prototype.construir = function(){
	...
	elemento.innerHTML = "<button type='button' menuBtn id='menuBtn'><i class='material-icons md-36 white' title='Menú'>menu</i></button><div titulo>SOCA PORTUGUESA</div>";
...
Cabecera.prototype.cambio_zafra = function() {
	var yo = this;
	var lista_zafra;
	var peticion = {
	   modulo: "agronomia",
	...
	torque
		...
		.then(function(respuesta){
			var zafras = respuesta.registros;
			var htmlNew='';
			for(var x=0;x<zafras.length;x++){
				htmlNew+='<button class="cambiozafra" valor="'+zafras[x].codigo+'" ';
				htmlNew+='>'+zafras[x].nombre.substr(0,4)+"-"+zafras[x].nombre.substr(4)+'</button>';
			}
			var ModalZafra = UI.crearVentanaModal({
				cabecera:{
					html:'Elija una Zafra',
				},
				cuerpo:{
					html:htmlNew
				}
			});
			ModalZafra.nodo.querySelectorAll('button.cambiozafra').forEach(function(boton){
				boton.onclick = function(){
					yo.elegir_zafra(this);
				}
			})
		});
}
Cabecera.prototype.elegir_zafra = function(zaf){
	var yo = this;
	document.getElementById("cam_zafra").innerHTML ="ZAFRA: "+ zaf.innerHTML;
	sesion.zafra = {
		codigo: zaf.getAttribute('valor'),
		nombre: zaf.innerHTML.substr(0,4)+zaf.innerHTML.substr(5)
	}
	var peticion = {
	   modulo: "agronomia",
	   entidad: "zafra",
	   operacion: "actualizarZafra",
	   codigo: sesion.zafra.codigo,
		 nombre: sesion.zafra.nombre
	};
	torque
		.Operacion(peticion)
		.then(function(respuesta){
...
var Menu = function(){
	...
	this.construir = function(){
		...
		if(typeof(sesion)!=='undefined'){
			...
			html+='<a class="pointer"><article  onclick="location.href=\'../../global/vistas/vis_Landing.html\'"><i class="material-icons md-48 white" title="Inicio">home</i></article></a>';
			html+='<a class="pointer"><article off ><i onclick="sesion.cerrarSesion()" title="Cerrar sesión"></i></article></a>';

		}
		html+='<a href=\'../../seguridad/vistas/vis_Cuenta.html\' class="pointer" title="Seguridad"><article seguridad><i></i></article></a>';
...
var Botonera = function(atributos){
	...
	this.buscarBotonEstructural = function(consBoton){
		var btn;
		if(typeof consBoton == 'string'){
			consBoton = {tipo:consBoton};
		}
		this.estructura.forEach(function(struc){
			if(struc.tipo==consBoton.tipo){
				btn = struc;
			}
		});
		return btn;
	...
	var capaContenido = function(){
		...
		var Pie = function(contenido){
			...
			this.detectarBotones = function(){
				this.botones = [];
				var self = this;
				this.nodo.querySelectorAll('button[nombre]').forEach(function(nodo){
					self.botones.push({
						"nodo": nodo,
...
var Lista = function(data){
  ...
  Lista.prototype.actualizarLista = function(cambios){
    if(cambios instanceof Array){
      ...
      return this.actualizarSlot(cambios);
    }
  };
...
construirCapaSelect= function(capaSelect){
	...
	var opcion = null;
	var nodo = null;
	capaSelect.parentNode.style.zIndex = "9999999";
