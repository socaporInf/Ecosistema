...
var Motor = function(moduloActivo,entidadActiva){
	...
	this.pedirReporte = function(datos){
		...
		if(presentacion === 'E'){
		    ...
		    var req = createCORSRequest('POST','http://'+document.domain+':8080/Ecosistema'+datos.ruta);
		    req.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
		    req.onreadystatechange = function(){
		      if (req.readyState == 4){
		        if(req.status !== 200){
				...
				req.send("agrupacion="+encodeURIComponent(JSON.stringify(datos.agr))+"&json=" + encodeURIComponent(JSON.stringify(datos.data)));
