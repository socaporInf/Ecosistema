...
var Maestro = function(atributos){
	...
	//-------------------------------------Manejo de UI-----------------------------------------------------------
	this.consultar = function(slot){
		return this.agregarFormulario(slot.atributos);
	}
	this.agregarLista = function (){
		var yo = this;
		var consLista = this.estructuraLista(this.entidadActiva,function(slot){
			if(yo.atributos.lista.onclickSlot){
				yo.atributos.lista.onclickSlot(slot);
			}else{
				yo.consultar(slot);
			}
		});
	...
	this.cambiosModificar = function(atributos){
		...
		return torque
			.manejarOperacion(peticion,cuadro)
	...
	this.buscarBoton = function(tipo){
		...
		if(!boton){
			boton = UI.elementos.botonera.buscarBotonEstructural(tipo);
		}
		return boton;
	};
